  ...
  try {
    ...
    const res = await pool.query(
      `SELECT id, name, slug, landing_url_template, pubid, postback_key, payout, currency, is_active, created_at, updated_at
       FROM adv_publishers ${where} ORDER BY created_at DESC`,
      params,
    );
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const res = await pool.query(
      `INSERT INTO adv_publishers (name, slug, landing_url_template, pubid, postback_key, payout, currency, is_active)
